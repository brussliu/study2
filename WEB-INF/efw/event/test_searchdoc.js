var test_searchdoc = {};
test_searchdoc.name = "資料情報検索";
test_searchdoc.paramsFormat = {
	"#opt_div1" : null,
	"#opt_div2" : null,
	"#opt_div3" : null,
	"#opt_div4" : null,
...
test_searchdoc.fire = function (params) {
	...
	for(var i = 0; i < selectResult.length; i++){
		...
		for(var j = 0; j < selectResult2.length; j++){
			...
			if(isPic(suffix)){
				subhtml = subhtml + "<img src='" + selectResult2[j]["content_tb50"] + "' class='selectimg'>" +
				"<input type='hidden' value='"+ selectResult2[j]["doc_no"] + "," + selectResult2[j]["doc_sub_no"] +"'/>&nbsp;";
			}else{
				subhtml = subhtml + "<img src='img/" + suffix + ".png' class='selectfile'>" +
		...
		var resultHTML =
		"<tr>" +
		"	<td rowspan='4' style='width: 50px;text-align: center;'><input type='checkbox' value=''/></td>" +
		"	<td rowspan='4' style='width: 150px;' class='c'>{doc_no}</td>" +
		"	<td rowspan='4' style='width: 100px;' class='l'>{status}</td>" +
		"	<td rowspan='4' style='width: 120px;text-align: center;'>{doc_vp}</td>" +
		"	<td style='width: 200px;'>{div1}</td>" +
		"	<td rowspan='4' style='width: 780px;'>{comment}<br>" + subhtml + "</td>" +
		"</tr>" +
		"<tr>" +
